...
func run(rules string, messages string) {
	rr := parseRules(rules)

	var matches int
	for _, message := range strings.Split(messages, "\n") {
		options := rr.match(0, message)
...
func parseRules(in string) ruleset {
	lines := strings.Split(in, "\n")
	rules := make(ruleset, 0, len(lines))
...
func parseRule(in string) rule {
	...
	if parts[1][0] == '"' {
		return rule{
			ruleNumber:  num,
			matchLetter: parts[1][1],
		}
	}
	parts = strings.Split(parts[1], " | ")
	or := make([][]int, 0, len(parts))
	for _, part := range parts {
		nums := strings.Split(part, " ")
		ii := make([]int, 0, len(nums))
		for _, nn := range nums {
			i, err := strconv.Atoi(nn)
			if err != nil {
				panic(err)
			}
			ii = append(ii, i)
...
type rule struct {
	ruleNumber int
	// if not zero the input must match this char
	matchLetter uint8
	// Sets of sub-rules. input can match any of them. Each subrule is a list
	// of rules that must all match
	or [][]int
}

type ruleset []rule

func (rs ruleset) match(i int, msg string) (options []int) {
	if len(msg) == 0 {
		return nil
	}
	...
	if r.matchLetter != 0 {
		if r.matchLetter == msg[0] {
			return []int{1}
		}
		return nil
	}

	for _, subRule := range r.or {

		// Need to schedule all possible amounts of consume through these rules
		consumed := []int{0}
		var next []int
		for _, j := range subRule {
			for _, c := range consumed {
				subc := rs.match(j, msg[c:])
				for i := range subc {
					subc[i] += c
				}
				next = append(next, subc...)
			}
			consumed, next = next, consumed
			next = next[:0]
		}
		options = append(options, consumed...)
	}
	return options
}

var rules = `19: 33 53 | 123 7
3: 33 82 | 123 45
92: 47 123 | 91 33
34: 123 60 | 33 63
91: 123 9 | 33 7
...
var messages = `aababbaabaabbaaaaabaaabb
