...
func run(input string) {
	...
	fmt.Println(w.countOccupied())

	w2 := new(input)
	for w2.update2() {
	}
	fmt.Println(w2.countOccupied())
...
func new(input string) world {
	lines := bytes.Split([]byte(input), sep)
	...
	return world{w: w, h: h, current: lines, next: next}
}

func (w world) countOccupied() int {
	o := 0
	for y := 0; y < w.h; y++ {
		for x := 0; x < w.w; x++ {
			if w.current[y][x] == '#' {
...
func (w world) canFill(x, y int) bool {
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			if dx == 0 && dy == 0 {
				continue
...
func (w world) canFill2(x, y int) bool {
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			if dx == 0 && dy == 0 {
				continue
...
func (w world) canEmpty(x, y int) bool {
	var occupied int
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			if dx == 0 && dy == 0 {
...
func (w world) canEmpty2(x, y int) bool {
	var occupied int
	for dy := -1; dy <= 1; dy++ {
		for dx := -1; dx <= 1; dx++ {
			if dx == 0 && dy == 0 {
...
func (w *world) update() (changed bool) {
	changes := 0
	for y := 0; y < w.h; y++ {
		for x := 0; x < w.w; x++ {
			switch v := w.at(x, y); v {
			case 'L':
				// look to see if we can place someone
				if w.canFill(x, y) {
					v = '#'
					changes++
				}
	...
	w.next, w.current = w.current, w.next
	return changes != 0
}

func (w *world) update2() (changed bool) {
	changes := 0
	for y := 0; y < w.h; y++ {
		for x := 0; x < w.w; x++ {
			switch v := w.at(x, y); v {
...
var input = `L.LLLLLLLLLL.LLLLLLLLL.LLLLLLLLL.LLL.LL.LLLLLLLLL.LLLLLL.LLLL.LL..LLLL.LLLL.LLL.LL.LLLLLLL
