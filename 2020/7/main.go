...
func run(input string) {
	...
	colours := make(map[string]int, len(lines))
	bags := make([]bag, len(lines))

	getColour := func(in string) int {
		colour, ok := colours[in]
		if !ok {
			colour = len(colours)
			colours[in] = colour
		}
	...
	for _, line := range lines {
		p := strings.Split(line, " bags contain ")
		colour := getColour(p[0])
		paren := getBag(colour)
		paren.colour = colour // a bit pointless

		children := strings.Split(p[1], ", ")
		for _, child := range children {
			var count int
			var adj, col string
			fmt.Sscanf(child, "%d %s %s bag", &count, &adj, &col)
			ccol := getColour(adj + " " + col)
			bag := getBag(ccol)
			bag.colour = ccol
			bag.parents = append(bag.parents, parent{count: count, colour: colour})
			paren.children = append(paren.children, parent{count: count, colour: ccol})
		}
	}

	var containing func(col int, m map[int]struct{})
	containing = func(col int, m map[int]struct{}) {
		bag := getBag(col)
		for _, col := range bag.parents {
			fmt.Println("col", col)
			m[col.colour] = struct{}{}
			containing(col.colour, m)
		}
	}

	m := make(map[int]struct{})
	containing(getColour("shiny gold"), m)
	fmt.Println(len(m))

	var addUp func(col int) int
	addUp = func(col int) int {
		b := getBag(col)
		total := 1
		for _, col := range b.children {
			total += col.count * addUp(col.colour)
		}
		return total
	}

	fmt.Println(addUp(getColour("shiny gold")) - 1)
}

var input = `dull blue bags contain 2 dotted green bags, 1 dull brown bag, 3 striped tomato bags, 5 muted blue bags.
