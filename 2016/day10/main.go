...
func (f *factory) parseLine(line string) {
	...
	case "bot":
		...
		lowIsOutput, lowTo := f.parseOutput(fields[5], fields[6])
		highIsOutput, highTo := f.parseOutput(fields[10], fields[11])
		f.bots[id] = bot{
			id:           id,
			lowTo:        lowTo,
	...
	case "value":
		...
		f.values = append(f.values, value{val: val, bot: id})
	}
}

func (*factory) parseOutput(outputType, number string) (bool, int) {
	isOutput := outputType == "output"
	outputNumber, _ := strconv.Atoi(number)
	return isOutput, outputNumber
}

func (f *factory) input(b *bot, value int) {
	b.values = append(b.values, value)
	if len(b.values) == 2 {
		l, h := b.values[0], b.values[1]
...
func (f *factory) output(id int, value int) {
	if id < 3 {
		f.outputMult *= value
	}
}
...
func (f *factory) run() {
	for _, value := range f.values {
		bot := &f.bots[value.bot]
		f.input(bot, value.val)
	}
}

func main() {
	f := &factory{outputMult: 1}
