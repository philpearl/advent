...
func supportsSSL(input string) bool {
	...
	hypernet := false
	babedAbas := make(map[string]struct{})
	babs := make([]string, 0)
	...
	for i, c := range input {
		...
		default:
			if i > 1 {
				candidate := input[i-2 : i+1]
				if !hypernet {
					if isAba(candidate) {
						babedAbas[abaToBab(candidate)] = struct{}{}
					}
				} else {
					if isAba(candidate) {
						babs = append(babs, candidate)
					}
				}
			}
		}
	...
	for _, bab := range babs {
		if _, present = babedAbas[bab]; present {
			break
		}
	}
...
func isAba(candidate string) bool {
...
func isAbba(candidate string) bool {
	return candidate[0] != candidate[1] &&
		candidate[0] == candidate[3] &&
		candidate[1] == candidate[2]
}

func assertTLS(input string, exp bool) {
	actual := supportsTLS(input)
	if actual != exp {
		fmt.Printf("%s TLS=%t, expected=%t\n", input, actual, exp)
	}
}

func assertSSL(input string, exp bool) {
	actual := supportsSSL(input)
	if actual != exp {
		fmt.Printf("%s SSL%t, expected=%t\n", input, actual, exp)
	}
}

func main() {
	assertTLS("abba[mnop]qrst", true)
	assertTLS("abcd[bddb]xyyx", false)
	assertTLS("aaaa[qwer]tyui", false)
	assertTLS("ioxxoj[asdfgh]zxcvbn", true)
	...
	assertSSL("aba[bab]xyz", true)
	assertSSL("xyx[xyx]xyx", false)
	assertSSL("aaa[kek]eke", true)
	assertSSL("zazbz[bzb]cdb", true)
