...
func addLetter(ll letters, toAdd rune) letters {
	for i := range ll {
		l := &ll[i]
	...
	return append(ll, letter{letter: toAdd, count: 1})
}

func (ll letters) top5() string {
	sort.Sort(ll)

	buf := &bytes.Buffer{}
	if len(ll) < 5 {
		fmt.Printf("ll too short. have %#v\n", ll)
	}
	for _, l := range ll[:5] {
		buf.WriteRune(l.letter)
	}
	return buf.String()
}

func parseName(name string) (ll letters, roomName string, sectorId int, checksum string) {
	inChecksum := false
	...
	checksum = checksumBuf.String()
	roomName = nameBuf.String()
	return
}

func decode(name string, rotate int) string {
	output := make([]byte, len(name))
	for i, x := range name {
		if x == ' ' {
			output[i] = byte(x)
		} else {
			output[i] = byte('a' + ((int(x-'a') + rotate) % 26))
		}
	}
...
func main() {
	...
	ll, roomName, sectorId, checksum := parseName("aaaaa-bbb-z-y-x-123[abxyz]")
	fmt.Printf("%s %s %d %s\n", roomName, ll.top5(), sectorId, checksum)
	ll, roomName, sectorId, checksum = parseName("a-b-c-d-e-f-g-h-987[abcde]")
	fmt.Printf("%s %d %s\n", ll.top5(), sectorId, checksum)
	ll, roomName, sectorId, checksum = parseName("not-a-real-room-404[oarel]")
	fmt.Printf("%s %d %s\n", ll.top5(), sectorId, checksum)
	ll, roomName, sectorId, checksum = parseName("totally-real-room-200[decoy]")
	fmt.Printf("%s %d %s\n", ll.top5(), sectorId, checksum)
	...
	for _, line := range strings.Split(input, "\n") {
		...
		ll, roomName, sectorId, checksum := parseName(line)

		if ll.top5() == checksum {
			sectorSum += sectorId
			decoded := decode(roomName, sectorId)
			if strings.Contains(decoded, "north") {
				fmt.Printf("%s %d\n", decoded, sectorId)
			}
		}
	}
	fmt.Printf("sector sum is %d\n", sectorSum)
