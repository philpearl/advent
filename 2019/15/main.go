...
type explore struct {
	...
	nextFrontier []pos
	depth        int
	// Visited map says how we got to each position. Following back should be shortest route
	visited map[pos]pos
	oxygen  pos
}
...
func (e *explore) output(o int) {
	...
	switch o {
	case 0:
		// Hit wall. Have not moved
		e.world[e.nextPosition()] = '#'
	case 1:
		// moved OK
		e.curr = e.nextPosition()
		if _, ok := e.world[e.curr]; !ok {
			e.world[e.curr] = '.'
			// New bit of world found. Add candidates for next exploration
			e.addCandidates()
		}
	case 2:
		// moved OK, oxygen
		fmt.Printf("Oxygen found at depth %d\n", e.depth)
		e.curr = e.nextPosition()
		e.world[e.curr] = 'o'
		e.oxygen = e.curr
	}
...
func (e *explore) setInput() {
	e.computer.input = 0
	for e.computer.input == 0 {
		// Decide on next input
		if len(e.route) != 0 {
			e.computer.input = e.route[0]
			e.route = e.route[1:]
			return
		}

		if len(e.frontier) == 0 {
			e.nextFrontier, e.frontier = e.frontier, e.nextFrontier
			e.nextFrontier = e.nextFrontier[:0]
			e.depth++
		}

		if len(e.frontier) == 0 {
			fmt.Printf("nothing to do (depth %d)\n", e.depth)
			break
		}
		...
		// Navigate to target
		e.route = navigate(e.curr, target, e.world)
		fmt.Println("route", e.route)
	}
}

func (e *explore) addCandidates() {
	// Add candidate positions around target to the next frontier
	for _, v := range []pos{{1, 0}, {-1, 0}, {0, 1}, {0, -1}} {
		candidate := pos{x: e.curr.x + v.x, y: e.curr.y + v.y}
		if _, ok := e.visited[candidate]; ok {
			continue
		}
		e.visited[candidate] = e.curr
		e.nextFrontier = append(e.nextFrontier, candidate)
	}
}

func (e *explore) nextPosition() pos {
	switch command(e.computer.input) {
	case north:
		return pos{x: e.curr.x, y: e.curr.y - 1}
	case south:
		return pos{x: e.curr.x, y: e.curr.y + 1}
	case east:
		return pos{x: e.curr.x + 1, y: e.curr.y}
	case west:
		return pos{x: e.curr.x - 1, y: e.curr.y}
	}
	panic(fmt.Sprintf("bad command %d", e.computer.input))
}
...
func run1(input []int) {
	...
	e.init(input)
	e.computer.input = north
	e.frontier = append(e.frontier, pos{x: 0, y: 0})
	e.computer.run()
...
// Navigate lets us get to a particular place via our explorered map
func navigate(start, target pos, world map[pos]byte) []int {
	type step struct {
		parent   pos
		location pos
	}
...
OUT:
	for len(q) > 0 {
		for _, item := range q {
			// Find all possible steps from here
			for _, v := range []pos{{1, 0}, {-1, 0}, {0, 1}, {0, -1}} {
				candidate := pos{x: item.x + v.x, y: item.y + v.y}
				if _, ok := visited[candidate]; ok {
					continue
				}
				visited[candidate] = item
				if candidate == target {
					fmt.Printf("found target %v from %v\n", target, start)
					found = true
					break OUT
				}
				if w := world[candidate]; w != '.' && w != 'o' && w != 's' {
					// Either a wall or unknown territory (we're not exploring)
					continue
				}
	...
	if !found {
		panic(fmt.Sprintf("no route from %v to %v (depth %d)", start, target, depth))
	}
	...
	for cursor != start {
		next := visited[cursor]
		// we want to move from next to cursor
		switch {
		case next.x > cursor.x:
			steps = append(steps, west)
		case next.x < cursor.x:
			steps = append(steps, east)
		case next.y > cursor.y:
...
func printArena(arena map[pos]byte) {
	var w, h int
	for p := range arena {
		if p.x < 0 {
			p.x = -p.x
