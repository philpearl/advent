...
func part1(input string) error {
	...
	for _, line := range lines {
		if len(line.pre) != 10 {
			return fmt.Errorf("bad pre")
		}
		if len(line.post) != 4 {
			return fmt.Errorf("bad post")
		}
		for _, n := range line.post {
			count += identifiable[bits.OnesCount8(n)]
		}
	}
...
func decodeLine(l line) int {
	...
	for _, p := range l.pre {
		c := bits.OnesCount8(p)
		byLen[c] = append(byLen[c], p)
	}

	var numbers [10]uint8
	numbers[1] = byLen[2][0]
	numbers[7] = byLen[3][0]
	numbers[4] = byLen[4][0]
	numbers[8] = byLen[7][0]

	// 6 digits are 0, 6 and 9
	for _, b := range byLen[6] {
		if b&numbers[7] == numbers[7] {
			// Could be 9 or zero
			if b&numbers[4] == numbers[4] {
				numbers[9] = b
			} else {
				numbers[0] = b
			}
	...
	// 5 is contained in 6
	// 2 is the other one!
	for _, b := range byLen[5] {
		switch {
		case b&numbers[7] == numbers[7]:
			numbers[3] = b
		case b&numbers[6] == b:
			numbers[5] = b
		default:
...
func parseInput(input string) []line {
	...
	for _, l := range strings.Split(input, "\n") {
		...
		lines = append(lines, line{
			pre:  stringsToBits(strings.Fields(parts[0])),
			post: stringsToBits(strings.Fields(parts[1])),
		})
	}
	return lines
}

func stringsToBits(in []string) []uint8 {
	out := make([]uint8, len(in))
	for i, in := range in {
		out[i] = stringToBits(in)
	}
	return out
}

func stringToBits(in string) (bits uint8) {
	for _, c := range in {
		bits |= 1 << (c - 'a')
	}
	return bits
...
var input = `ceb bgfdea febgc ec eadcgfb eagbcd fcdebg dcef gafbc egdbf | fdbgec fedbg gdabefc gefbd
