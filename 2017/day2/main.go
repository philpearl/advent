...
func main() {
	...
	fmt.Printf("evenDivisors is %d\n", s.dividorsSum())
}

type row []int
type sheet []row

func (s sheet) checksum() int {
	t := 0
	for _, r := range s {
		t += r.diff()
	}
...
func parseInput(input string) sheet {
	lines := strings.Split(input, "\n")
	s := make(sheet, 0, len(lines))
	for _, line := range lines {
		cells := strings.Split(line, "\t")
		r := make(row, 0, len(cells))
