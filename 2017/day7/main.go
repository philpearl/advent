...
func main() {
	...
	// start = time.Now()
	// part1_2()
	// fmt.Printf("p1_2 in %s\n", time.Since(start))
}

func part1() *node {
	// alternatively you could find any node that isn't in the children lists
	ns := parseInput(input)
	...
	for lastN.parent != nil {
		lastN = lastN.parent
	}
	fmt.Printf("Top node is %s\n", lastN.id)
	return lastN
}

func part2(top *node) {

	top.balancedChildren()
}

func (n *node) balancedChildren() (balanced bool, weight int) {
	w := make([]int, len(n.childNodes))
	balanced = true
	for i, c := range n.childNodes {
		_, cw := c.balancedChildren()
		weight += cw
		w[i] = cw
		if w[i] != w[0] {
			balanced = false
	...
	if !balanced {
		fmt.Printf("Node %s not balanced\n", n.id)
		for i, c := range n.childNodes {
			fmt.Printf("  %s weight %d (own weight %d)\n", c.id, w[i], c.weight)
		}
	}

	return balanced, weight + n.weight
}

func parseInput(input string) nodes {
	lines := strings.Split(input, "\n")
	ns := make(nodes, len(lines))
	for _, line := range lines {
		...
		if len(fields) > 2 {
			n.children = append(n.children, fields[3:]...)
		}
		ns[n.id] = n
	}
...
func parseWeight(w string) int {
	v, err := strconv.Atoi(w[1 : len(w)-1])
	if err != nil {
		panic(err)
	}
	return v
...
func part1_2() {
	parents, children := parseInput2(input)
	sort.Strings(parents)
	sort.Strings(children)
	for i, p := range parents {
		if children[i] != p {
			fmt.Printf("Version 2 top is %s\n", p)
			break
		}
	}
}

func parseInput2(input string) (parents, children []string) {
	lines := strings.Split(input, "\n")
	...
	for _, line := range lines {
		...
		if len(fields) > 2 {
			children = append(children, fields[3:]...)
		}
	}
