...
func part1(input string) {
	...
	fmt.Printf("Largest register is %s with %d\n", bk, bv)
	fmt.Printf("largest value %d\n", m.highest)
}

func (m *machine) runProgram(cmds []command) {
	for _, c := range cmds {
		m.run(c)
	}
}

func parseProgram(p string) []command {
...
func opFromString(in string) op {
	switch in {
	case "inc":
		return inc
	case "dec":
...
func (c cond) isTrue(l, r int) bool {
	switch c {
	case ne:
		return l != r
	case eq:
...
func condFromString(in string) cond {
	switch in {
	case "!=":
		return ne
	case "==":
...
func (m *machine) run(c command) {
	condRegVal := m.registers[c.condRegister]
	if c.cond.isTrue(condRegVal, c.condVal) {
		switch c.op {
		case inc:
			m.registers[c.register] += c.opVal
		case dec:
			m.registers[c.register] -= c.opVal
		}

		if m.registers[c.register] > m.highest {
			m.highest = m.registers[c.register]
		}
	}
}
...
func parseCommand(cmd string) command {
	...
	return command{
		register:     regname(fields[0]),
		op:           opFromString(fields[1]),
		opVal:        mustParseInt(fields[2]),
		condRegister: regname(fields[4]),
		cond:         condFromString(fields[5]),
		condVal:      mustParseInt(fields[6]),
	}
}

var input = `q inc -541 if c != 4
 s inc -555 if o > -5
 px dec -84 if x >= -4
 y dec -822 if txk > -1
