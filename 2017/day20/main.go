...
func part2(input string) {
	ps := parseParticles(input)

	for tick := 0; ; tick++ {
		ps = removeCollisions(ps)
		fmt.Printf("%d particles left\n", len(ps))
...
func removeCollisions(ps []particle) []particle {
	...
	nps := make([]particle, 0, len(ps))
	var currentp vec
	currentStart := -2
	for i := range ps {
		p := ps[i].p
		if p != currentp || currentStart < 0 {
			// Start of a new range of identical ps
			// Finish processing old range
			if currentStart == i-1 {
				// Copy the last range over
				nps = append(nps, ps[currentStart])
			}
...
func part1(input string) {
	...
	sort.Slice(ps, func(i, j int) bool {
		pi, pj := &ps[i], &ps[j]
		ami, amj := pi.a.manhattan(), pj.a.manhattan()
		if ami == amj {
			vmi, vmj := pi.v.manhattan(), pj.v.manhattan()
			if vmi == vmj {
				pmi, pmj := pi.p.manhattan(), pj.p.manhattan()
				return pmi < pmj
			}
			return vmi < vmj
		}
...
func parseParticles(text string) []particle {
	lines := strings.Split(text, "\n")
	ps := make([]particle, len(lines))
	for i, line := range lines {
		ps[i] = parseParticle(line, i)
	}
	return ps
}

func parseParticle(line string, id int) particle {
...
var input = `p=<-317,1413,1507>, v=<19,-102,-108>, a=<1,-3,-3>
