...
func main() {
	...
	for _, line := range input {
		minute := line[minStartO : minStartO+2]
		m, _ := strconv.Atoi(minute)

		action := line[actionStartO:]
		switch action {
		case "falls asleep":
			sleepStart = m
		case "wakes up":
	...
	fmt.Println(maxGuard, maxMin, maxSleep)
	fmt.Println(g)
...
const (
	minStart  = "[1518-05-27 00:"
	minStartO = len(minStart)

	actionStart  = "[1518-05-27 00:42] "
	actionStartO = len(actionStart)
)
