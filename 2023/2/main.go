...
func parseGame(line string) (int, []round) {
	game, rest, _ := strings.Cut(line, ": ")

	var id int
	fmt.Sscanf(game, "Game %d", &id)

	roundss := strings.Split(rest, "; ")
	rounds := make([]round, len(roundss))

	for _, r := range roundss {
		colours := strings.Split(r, ", ")
		var round round
		for _, c := range colours {
			var val int
			var colour string
			n, err := fmt.Sscanf(c, "%d %s", &val, &colour)
			if err != nil {
				panic(err)
			}
			if n != 2 {
...
var input = `Game 1: 1 blue; 4 green, 5 blue; 11 red, 3 blue, 11 green; 1 red, 10 green, 4 blue; 17 red, 12 green, 7 blue; 3 blue, 19 green, 15 red
